...
***************************************************************************************/
  void TFT_eSPI_RA8876::spi_begin()       {begin_tft_write();}
  void TFT_eSPI_RA8876::spi_end()         {  end_tft_write();}
  void TFT_eSPI_RA8876::spi_begin_read()  {begin_tft_read(); }
  void TFT_eSPI_RA8876::spi_end_read()    {  end_tft_read(); }

/***************************************************************************************
** Function name:           begin_tft_write (was called spi_begin)
** Description:             Start SPI transaction for writes and select TFT
***************************************************************************************/
inline void TFT_eSPI_RA8876::begin_tft_write(void){
  if (locked) {
...
void TFT_eSPI_RA8876::begin_nin_write(void){
  if (locked) {
    locked = false; // Flag to show SPI access now unlocked
#if defined (SPI_HAS_TRANSACTION) && defined (SUPPORT_TRANSACTIONS) && !defined(TFT_PARALLEL_8_BIT) && !defined(RP2040_PIO_INTERFACE)
    spi.beginTransaction(SPISettings(SPI_FREQUENCY, MSBFIRST, TFT_SPI_MODE));
...
***************************************************************************************/
inline void TFT_eSPI_RA8876::end_tft_write(void){
...
// Non-inlined version to permit override
inline void TFT_eSPI_RA8876::end_nin_write(void){
  if(!inTransaction) {      // Flag to stop ending transaction during multiple graphics calls
    if (!locked) {          // Locked when beginTransaction has been called
      locked = true;        // Flag to show SPI access now locked
      SPI_BUSY_CHECK;       // Check send complete and clean out unused rx data
...
inline void TFT_eSPI_RA8876::begin_tft_read(void){
...
***************************************************************************************/
inline void TFT_eSPI_RA8876::end_tft_read(void){
#if defined (SPI_HAS_TRANSACTION) && defined (SUPPORT_TRANSACTIONS) && !defined(TFT_PARALLEL_8_BIT) && !defined(RP2040_PIO_INTERFACE)
  if(!inTransaction) {
    if (!locked) {
      locked = true;
...
***************************************************************************************/
void TFT_eSPI_RA8876::setViewport(int32_t x, int32_t y, int32_t w, int32_t h, bool vpDatum)
{
...
bool TFT_eSPI_RA8876::checkViewport(int32_t x, int32_t y, int32_t w, int32_t h)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::resetViewport(void)
{
...
***************************************************************************************/
int32_t  TFT_eSPI_RA8876::getViewportX(void)
{
  return _xDatum;
}
...
***************************************************************************************/
int32_t  TFT_eSPI_RA8876::getViewportY(void)
{
  return _yDatum;
}
...
***************************************************************************************/
int32_t TFT_eSPI_RA8876::getViewportWidth(void)
{
  return _xWidth;
}
...
***************************************************************************************/
int32_t TFT_eSPI_RA8876::getViewportHeight(void)
{
  return _yHeight;
}
...
***************************************************************************************/
bool  TFT_eSPI_RA8876::getViewportDatum(void)
{
  return _vpDatum;
}
...
***************************************************************************************/
void TFT_eSPI_RA8876::frameViewport(uint16_t color, int32_t w)
{
  // Save datum position
  bool _dT = _vpDatum;
...
***************************************************************************************/
bool TFT_eSPI_RA8876::clipAddrWindow(int32_t *x, int32_t *y, int32_t *w, int32_t *h)
{
...
***************************************************************************************/
bool TFT_eSPI_RA8876::clipWindow(int32_t *xs, int32_t *ys, int32_t *xe, int32_t *ye)
{
  if (_vpOoB) return false; // Area is outside of viewport
...
/***************************************************************************************
** Function name:           TFT_eSPI_RA8876
** Description:             Constructor , we must use hardware SPI pins
***************************************************************************************/
TFT_eSPI_RA8876::TFT_eSPI_RA8876(int16_t w, int16_t h)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::initBus(void) {
...
***************************************************************************************/
void TFT_eSPI_RA8876::begin(uint8_t tc)
{
 init(tc);
}
...
***************************************************************************************/
void TFT_eSPI_RA8876::init(uint8_t tc)
{
  if (_booted)
  {
    initBus();
...
***************************************************************************************/
void TFT_eSPI_RA8876::setRotation(uint8_t m)
{
...
***************************************************************************************/
uint8_t TFT_eSPI_RA8876::getRotation(void)
{
  return rotation;
}
...
void TFT_eSPI_RA8876::setOrigin(int32_t x, int32_t y)
{
  _xDatum = x;
  _yDatum = y;
}
...
***************************************************************************************/
int32_t TFT_eSPI_RA8876::getOriginX(void)
{
  return _xDatum;
}
...
***************************************************************************************/
int32_t TFT_eSPI_RA8876::getOriginY(void)
{
  return _yDatum;
}
...
***************************************************************************************/
void TFT_eSPI_RA8876::commandList (const uint8_t *addr)
{
  uint8_t  numCommands;
  uint8_t  numArgs;
  uint8_t  ms;
...
***************************************************************************************/
void TFT_eSPI_RA8876::spiwrite(uint8_t c)
{
  begin_tft_write();
  tft_Write_8(c);
  end_tft_write();
...
***************************************************************************************/
// RA8876 has no DC pin: a prefix byte (0x00 for address, 0x80 for data) is sent
// before each payload byte, and CS must toggle for every 2-byte transaction.
...
#else
void TFT_eSPI_RA8876::writecommand(uint16_t c)
{
  begin_tft_write();
...
void TFT_eSPI_RA8876::writeRegister8(uint16_t c, uint8_t d)
{
  begin_tft_write();
...
void TFT_eSPI_RA8876::writeRegister16(uint16_t c, uint16_t d)
{
  begin_tft_write();
...
***************************************************************************************/
#ifndef RA8876_DRIVER   // RA8876 writedata is defined alongside writecommand above
void TFT_eSPI_RA8876::writedata(uint8_t d)
{
  begin_tft_write();
...
***************************************************************************************/
uint8_t TFT_eSPI_RA8876::readcommand8(uint8_t cmd_function, uint8_t index)
{
  uint8_t reg = 0;
#if defined(TFT_PARALLEL_8_BIT) || defined(RP2040_PIO_INTERFACE)
...
***************************************************************************************/
uint16_t TFT_eSPI_RA8876::readcommand16(uint8_t cmd_function, uint8_t index)
{
  uint32_t reg;
...
***************************************************************************************/
uint32_t TFT_eSPI_RA8876::readcommand32(uint8_t cmd_function, uint8_t index)
{
  uint32_t reg;
...
***************************************************************************************/
uint16_t TFT_eSPI_RA8876::readPixel(int32_t x0, int32_t y0)
{
  if (_vpOoB) return 0;
...
void TFT_eSPI_RA8876::setCallback(getColorCallback getCol)
{
  getColor = getCol;
}
...
***************************************************************************************/
void TFT_eSPI_RA8876::readRect(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t *data)
{
  PI_CLIP ;
...
***************************************************************************************/
void TFT_eSPI_RA8876::pushRect(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t *data)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::pushImage(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t *data)
{
  PI_CLIP;
...
***************************************************************************************/
void TFT_eSPI_RA8876::pushImage(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t *data, uint16_t transp)
{
  PI_CLIP;
...
***************************************************************************************/
void TFT_eSPI_RA8876::pushImage(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t *data)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::pushImage(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t *data, uint16_t transp)
{
  // Requires 32-bit aligned access, so use PROGMEM 16-bit word functions
  PI_CLIP;
...
***************************************************************************************/
void TFT_eSPI_RA8876::pushImage(int32_t x, int32_t y, int32_t w, int32_t h, const uint8_t *data, bool bpp8,  uint16_t *cmap)
{
  PI_CLIP;
...
** Description:             plot 8-bit or 4-bit or 1 bit image or sprite using a line buffer
***************************************************************************************/
void TFT_eSPI_RA8876::pushImage(int32_t x, int32_t y, int32_t w, int32_t h, uint8_t *data, bool bpp8,  uint16_t *cmap)
{
  PI_CLIP;
...
***************************************************************************************/
void TFT_eSPI_RA8876::pushImage(int32_t x, int32_t y, int32_t w, int32_t h, uint8_t *data, uint8_t transp, bool bpp8, uint16_t *cmap)
{
  PI_CLIP;
...
void TFT_eSPI_RA8876::pushMaskedImage(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t *img, uint8_t *mask)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::setSwapBytes(bool swap)
{
  _swapBytes = swap;
}
...
***************************************************************************************/
bool TFT_eSPI_RA8876::getSwapBytes(void)
{
  return _swapBytes;
}
...
void  TFT_eSPI_RA8876::readRectRGB(int32_t x0, int32_t y0, int32_t w, int32_t h, uint8_t *data)
{
#if defined(TFT_PARALLEL_8_BIT) || defined(RP2040_PIO_INTERFACE)
...
void TFT_eSPI_RA8876::drawCircle(int32_t x0, int32_t y0, int32_t r, uint32_t color)
{
  if ( r <= 0 ) return;
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawCircleHelper( int32_t x0, int32_t y0, int32_t rr, uint8_t cornername, uint32_t color)
{
  if (rr <= 0) return;
  int32_t f     = 1 - rr;
  int32_t ddF_x = 1;
...
void TFT_eSPI_RA8876::fillCircle(int32_t x0, int32_t y0, int32_t r, uint32_t color)
{
  int32_t  x  = 0;
  int32_t  dx = 1;
  int32_t  dy = r+r;
...
void TFT_eSPI_RA8876::fillCircleHelper(int32_t x0, int32_t y0, int32_t r, uint8_t cornername, int32_t delta, uint32_t color)
{
  int32_t f     = 1 - r;
  int32_t ddF_x = 1;
  int32_t ddF_y = -r - r;
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawEllipse(int16_t x0, int16_t y0, int32_t rx, int32_t ry, uint16_t color)
{
  if (rx<2) return;
  if (ry<2) return;
  int32_t x, y;
...
***************************************************************************************/
void TFT_eSPI_RA8876::fillEllipse(int16_t x0, int16_t y0, int32_t rx, int32_t ry, uint16_t color)
{
  if (rx<2) return;
  if (ry<2) return;
  int32_t x, y;
...
***************************************************************************************/
void TFT_eSPI_RA8876::fillScreen(uint32_t color)
{
...
***************************************************************************************/
// Draw a rectangle
void TFT_eSPI_RA8876::drawRect(int32_t x, int32_t y, int32_t w, int32_t h, uint32_t color)
{
  //begin_tft_write();          // Sprite class can use this function, avoiding begin_tft_write()
  inTransaction = true;
...
void TFT_eSPI_RA8876::drawRoundRect(int32_t x, int32_t y, int32_t w, int32_t h, int32_t r, uint32_t color)
{
  //begin_tft_write();          // Sprite class can use this function, avoiding begin_tft_write()
  inTransaction = true;
...
void TFT_eSPI_RA8876::fillRoundRect(int32_t x, int32_t y, int32_t w, int32_t h, int32_t r, uint32_t color)
{
  //begin_tft_write();          // Sprite class can use this function, avoiding begin_tft_write()
  inTransaction = true;
...
***************************************************************************************/
// Draw a triangle
void TFT_eSPI_RA8876::drawTriangle(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2, int32_t y2, uint32_t color)
{
  //begin_tft_write();          // Sprite class can use this function, avoiding begin_tft_write()
  inTransaction = true;
...
void TFT_eSPI_RA8876::fillTriangle ( int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2, int32_t y2, uint32_t color)
{
  int32_t a, b, y, last;
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color)
{
  //begin_tft_write();          // Sprite class can use this function, avoiding begin_tft_write()
  inTransaction = true;
...
** Description:             Draw an image stored in an array on the TFT
***************************************************************************************/
void TFT_eSPI_RA8876::drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t fgcolor, uint16_t bgcolor)
{
  //begin_tft_write();          // Sprite class can use this function, avoiding begin_tft_write()
  inTransaction = true;
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawXBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color)
{
  //begin_tft_write();          // Sprite class can use this function, avoiding begin_tft_write()
  inTransaction = true;
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawXBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color, uint16_t bgcolor)
{
  //begin_tft_write();          // Sprite class can use this function, avoiding begin_tft_write()
  inTransaction = true;
...
***************************************************************************************/
void TFT_eSPI_RA8876::setCursor(int16_t x, int16_t y)
{
  cursor_x = x;
  cursor_y = y;
}
...
***************************************************************************************/
void TFT_eSPI_RA8876::setCursor(int16_t x, int16_t y, uint8_t font)
{
  setTextFont(font);
  cursor_x = x;
  cursor_y = y;
...
***************************************************************************************/
int16_t TFT_eSPI_RA8876::getCursorX(void)
{
  return cursor_x;
}
...
***************************************************************************************/
int16_t TFT_eSPI_RA8876::getCursorY(void)
{
  return cursor_y;
}
...
***************************************************************************************/
void TFT_eSPI_RA8876::setTextSize(uint8_t s)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::setTextColor(uint16_t c)
{
...
// be used.
void TFT_eSPI_RA8876::setTextColor(uint16_t c, uint16_t b, bool bgfill)
{
  textcolor   = c;
  textbgcolor = b;
  _fillbg     = bgfill;
...
*************************************************************************************x*/
void TFT_eSPI_RA8876::setPivot(int16_t x, int16_t y)
{
  _xPivot = x;
  _yPivot = y;
}
...
***************************************************************************************/
int16_t TFT_eSPI_RA8876::getPivotX(void)
{
  return _xPivot;
}
...
***************************************************************************************/
int16_t TFT_eSPI_RA8876::getPivotY(void)
{
  return _yPivot;
}
...
***************************************************************************************/
void TFT_eSPI_RA8876::setBitmapColor(uint16_t c, uint16_t b)
{
  if (c == b) b = ~c;
  bitmap_fg = c;
  bitmap_bg = b;
...
***************************************************************************************/
void TFT_eSPI_RA8876::setTextWrap(bool wrapX, bool wrapY)
{
  textwrapX = wrapX;
  textwrapY = wrapY;
}
...
***************************************************************************************/
void TFT_eSPI_RA8876::setTextDatum(uint8_t d)
{
  textdatum = d;
}
...
***************************************************************************************/
void TFT_eSPI_RA8876::setTextPadding(uint16_t x_width)
{
  padX = x_width;
}
...
** Description:             Define padding width (aids erasing old text and numbers)
***************************************************************************************/
uint16_t TFT_eSPI_RA8876::getTextPadding(void)
{
  return padX;
}
...
***************************************************************************************/
uint8_t TFT_eSPI_RA8876::getTextDatum(void)
{
  return textdatum;
}
...
int16_t TFT_eSPI_RA8876::width(void)
{
...
***************************************************************************************/
int16_t TFT_eSPI_RA8876::height(void)
{
...
***************************************************************************************/
int16_t TFT_eSPI_RA8876::textWidth(const String& string)
{
...
int16_t TFT_eSPI_RA8876::textWidth(const String& string, uint8_t font)
{
  int16_t len = string.length() + 2;
  char buffer[len];
  string.toCharArray(buffer, len);
...
int16_t TFT_eSPI_RA8876::textWidth(const char *string)
{
  return textWidth(string, textfont);
}

int16_t TFT_eSPI_RA8876::textWidth(const char *string, uint8_t font)
{
  int32_t str_width = 0;
  uint16_t uniCode  = 0;
...
uint16_t TFT_eSPI_RA8876::fontsLoaded(void)
{
  return fontsloaded;
}
...
***************************************************************************************/
int16_t TFT_eSPI_RA8876::fontHeight(uint8_t font)
{
  if (font > 8) return 0;
...
int16_t TFT_eSPI_RA8876::fontHeight(void)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawChar(int32_t x, int32_t y, uint16_t c, uint32_t color, uint32_t bg, uint8_t size)
{
  if (_vpOoB) return;
...
void TFT_eSPI_RA8876::setWindow(int32_t x0, int32_t y0, int32_t x1, int32_t y1)
{
  ...
  { uint16_t _w = x1 - x0 + 1, _h = y1 - y0 + 1;
    RA8876_REG(RA8876_AW_WTH0,  _w & 0xFF);
    RA8876_REG(RA8876_AW_WTH1, (_w >> 8) & 0x0F);
    RA8876_REG(RA8876_AW_HT0,   _h & 0xFF);
    RA8876_REG(RA8876_AW_HT1,  (_h >> 8) & 0x0F);
  }
  ...
  // Select MRWDP; RA8876_CMD leaves CS HIGH
  RA8876_CMD(RA8876_MRWDP);
  // Re-assert CS and send burst data-write prefix — CS stays LOW for pixel streaming
  CS_L;
  tft_Write_8(0x80);
#else
  #if defined (SSD1963_DRIVER)
...
***************************************************************************************/
void TFT_eSPI_RA8876::readAddrWindow(int32_t xs, int32_t ys, int32_t w, int32_t h)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawPixel(int32_t x, int32_t y, uint32_t color)
{
  if (_vpOoB) return;
...
***************************************************************************************/
void TFT_eSPI_RA8876::pushColor(uint16_t color)
{
  begin_tft_write();
...
***************************************************************************************/
void TFT_eSPI_RA8876::pushColor(uint16_t color, uint32_t len)
{
  begin_tft_write();
...
***************************************************************************************/
void TFT_eSPI_RA8876::startWrite(void)
{
  begin_tft_write();
...
***************************************************************************************/
void TFT_eSPI_RA8876::endWrite(void)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::writeColor(uint16_t color, uint32_t len)
{
  pushBlock(color, len);
}
...
void TFT_eSPI_RA8876::pushColors(uint8_t *data, uint32_t len)
{
  begin_tft_write();
...
***************************************************************************************/
void TFT_eSPI_RA8876::pushColors(uint16_t *data, uint32_t len, bool swap)
{
  begin_tft_write();
...
void TFT_eSPI_RA8876::drawLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint32_t color)
{
  if (_vpOoB) return;
...
***************************************************************************************/
uint16_t TFT_eSPI_RA8876::drawPixel(int32_t x, int32_t y, uint32_t color, uint8_t alpha, uint32_t bg_color)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawSmoothArc(int32_t x, int32_t y, int32_t r, int32_t ir, uint32_t startAngle, uint32_t endAngle, uint32_t fg_color, uint32_t bg_color, bool roundEnds)
// Centre at x,y
...
inline uint8_t TFT_eSPI_RA8876::sqrt_fraction(uint32_t num) {
...
void TFT_eSPI_RA8876::drawArc(int32_t x, int32_t y, int32_t r, int32_t ir,
...
void TFT_eSPI_RA8876::drawSmoothCircle(int32_t x, int32_t y, int32_t r, uint32_t fg_color, uint32_t bg_color)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::fillSmoothCircle(int32_t x, int32_t y, int32_t r, uint32_t color, uint32_t bg_color)
{
  if (r <= 0) return;
...
//   0x8 | 0x4
void TFT_eSPI_RA8876::drawSmoothRoundRect(int32_t x, int32_t y, int32_t r, int32_t ir, int32_t w, int32_t h, uint32_t fg_color, uint32_t bg_color, uint8_t quadrants)
{
  if (_vpOoB) return;
  if (r < ir) transpose(r, ir); // Required that r > ir
...
***************************************************************************************/
void TFT_eSPI_RA8876::fillSmoothRoundRect(int32_t x, int32_t y, int32_t w, int32_t h, int32_t r, uint32_t color, uint32_t bg_color)
{
  inTransaction = true;
...
void TFT_eSPI_RA8876::drawSpot(float ax, float ay, float r, uint32_t fg_color, uint32_t bg_color)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawWideLine(float ax, float ay, float bx, float by, float wd, uint32_t fg_color, uint32_t bg_color)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawWedgeLine(float ax, float ay, float bx, float by, float ar, float br, uint32_t fg_color, uint32_t bg_color)
{
...
***************************************************************************************/
inline float TFT_eSPI_RA8876::wedgeLineDistance(float xpax, float ypay, float bax, float bay, float dr)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawFastVLine(int32_t x, int32_t y, int32_t h, uint32_t color)
{
  if (_vpOoB) return;
...
***************************************************************************************/
void TFT_eSPI_RA8876::drawFastHLine(int32_t x, int32_t y, int32_t w, uint32_t color)
{
  if (_vpOoB) return;
...
***************************************************************************************/
void TFT_eSPI_RA8876::fillRect(int32_t x, int32_t y, int32_t w, int32_t h, uint32_t color)
{
  if (_vpOoB) return;
...
***************************************************************************************/
void TFT_eSPI_RA8876::fillRectVGradient(int16_t x, int16_t y, int16_t w, int16_t h, uint32_t color1, uint32_t color2)
{
  if (_vpOoB) return;
...
***************************************************************************************/
void TFT_eSPI_RA8876::fillRectHGradient(int16_t x, int16_t y, int16_t w, int16_t h, uint32_t color1, uint32_t color2)
{
  if (_vpOoB) return;
...
***************************************************************************************/
uint16_t TFT_eSPI_RA8876::color565(uint8_t r, uint8_t g, uint8_t b)
{
...
***************************************************************************************/
uint8_t TFT_eSPI_RA8876::color16to8(uint16_t c)
{
...
***************************************************************************************/
uint16_t TFT_eSPI_RA8876::color8to16(uint8_t color)
{
  uint8_t  blue[] = {0, 11, 21, 31}; // blue 2 to 5-bit colour lookup table
  uint16_t color16 = 0;
...
***************************************************************************************/
uint32_t TFT_eSPI_RA8876::color16to24(uint16_t color565)
{
...
***************************************************************************************/
uint32_t TFT_eSPI_RA8876::color24to16(uint32_t color888)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::invertDisplay(bool i)
{
  begin_tft_write();
...
**************************************************************************/
void TFT_eSPI_RA8876::setAttribute(uint8_t attr_id, uint8_t param) {
    switch (attr_id) {
            break;
        case CP437_SWITCH:
            _cp437 = param;
...
**************************************************************************/
uint8_t TFT_eSPI_RA8876::getAttribute(uint8_t attr_id) {
    switch (attr_id) {
...
*************************************************************************************x*/
uint16_t TFT_eSPI_RA8876::decodeUTF8(uint8_t c)
{
  if (!_utf8) return c;
...
*************************************************************************************x*/
uint16_t TFT_eSPI_RA8876::decodeUTF8(uint8_t *buf, uint16_t *index, uint16_t remaining)
{
...
*************************************************************************************x*/
uint16_t TFT_eSPI_RA8876::alphaBlend(uint8_t alpha, uint16_t fgc, uint16_t bgc)
{
...
*************************************************************************************x*/
uint16_t TFT_eSPI_RA8876::alphaBlend(uint8_t alpha, uint16_t fgc, uint16_t bgc, uint8_t dither)
{
  if (dither) {
...
*************************************************************************************x*/
uint32_t TFT_eSPI_RA8876::alphaBlend24(uint8_t alpha, uint32_t fgc, uint32_t bgc, uint8_t dither)
{
...
size_t TFT_eSPI_RA8876::write(const uint8_t *buf, size_t len)
{
  inTransaction = true;
...
** Description:             draw characters piped through serial stream
***************************************************************************************/
size_t TFT_eSPI_RA8876::write(uint8_t utf8)
{
  if (_vpOoB) return 1;
...
***************************************************************************************/
  // TODO: Rationalise with TFT_eSprite_RA8876
  // Any UTF-8 decoding must be done before calling drawChar()
int16_t TFT_eSPI_RA8876::drawChar(uint16_t uniCode, int32_t x, int32_t y)
{
  return drawChar(uniCode, x, y, textfont);
}

  // Any UTF-8 decoding must be done before calling drawChar()
int16_t TFT_eSPI_RA8876::drawChar(uint16_t uniCode, int32_t x, int32_t y, uint8_t font)
{
...
int16_t TFT_eSPI_RA8876::drawString(const String& string, int32_t poX, int32_t poY)
{
  int16_t len = string.length() + 2;
  char buffer[len];
  string.toCharArray(buffer, len);
...
// With font number
int16_t TFT_eSPI_RA8876::drawString(const String& string, int32_t poX, int32_t poY, uint8_t font)
{
  int16_t len = string.length() + 2;
  char buffer[len];
  string.toCharArray(buffer, len);
...
// Without font number, uses font set by setTextFont()
int16_t TFT_eSPI_RA8876::drawString(const char *string, int32_t poX, int32_t poY)
{
  return drawString(string, poX, poY, textfont);
}

// With font number. Note: font number is over-ridden if a smooth font is loaded
int16_t TFT_eSPI_RA8876::drawString(const char *string, int32_t poX, int32_t poY, uint8_t font)
{
  if (font > 8) return 0;
...
***************************************************************************************/
int16_t TFT_eSPI_RA8876::drawCentreString(const String& string, int32_t dX, int32_t poY, uint8_t font)
{
  int16_t len = string.length() + 2;
  char buffer[len];
  string.toCharArray(buffer, len);
...
int16_t TFT_eSPI_RA8876::drawCentreString(const char *string, int32_t dX, int32_t poY, uint8_t font)
{
...
***************************************************************************************/
int16_t TFT_eSPI_RA8876::drawRightString(const String& string, int32_t dX, int32_t poY, uint8_t font)
{
  int16_t len = string.length() + 2;
  char buffer[len];
  string.toCharArray(buffer, len);
...
int16_t TFT_eSPI_RA8876::drawRightString(const char *string, int32_t dX, int32_t poY, uint8_t font)
{
  uint8_t tempdatum = textdatum;
  int16_t sumX = 0;
  textdatum = TR_DATUM;
...
***************************************************************************************/
int16_t TFT_eSPI_RA8876::drawNumber(long long_num, int32_t poX, int32_t poY)
{
...
int16_t TFT_eSPI_RA8876::drawNumber(long long_num, int32_t poX, int32_t poY, uint8_t font)
{
  isDigits = true; // Eliminate jiggle in monospaced fonts
  char str[12];
  ltoa(long_num, str, 10);
...
int16_t TFT_eSPI_RA8876::drawFloat(float floatNumber, uint8_t dp, int32_t poX, int32_t poY)
{
  return drawFloat(floatNumber, dp, poX, poY, textfont);
}

int16_t TFT_eSPI_RA8876::drawFloat(float floatNumber, uint8_t dp, int32_t poX, int32_t poY, uint8_t font)
{
  isDigits = true;
  ...
  // For error put ... in string and return (all TFT_eSPI_RA8876 library fonts contain . character)
...
void TFT_eSPI_RA8876::setFreeFont(const GFXfont *f)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::setTextFont(uint8_t f)
{
...
void TFT_eSPI_RA8876::setFreeFont(uint8_t font)
{
  setTextFont(font);
}
...
***************************************************************************************/
#if !defined (TFT_PARALLEL_8_BIT) && !defined (RP2040_PIO_INTERFACE)
SPIClass& TFT_eSPI_RA8876::getSPIinstance(void)
{
  return spi;
}
#endif
...
***************************************************************************************/
bool TFT_eSPI_RA8876::verifySetupID(uint32_t id)
{
...
***************************************************************************************/
void TFT_eSPI_RA8876::getSetup(setup_t &tft_settings)
{
